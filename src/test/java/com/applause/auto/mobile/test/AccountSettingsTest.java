...
public class AccountSettingsTest extends BaseTest {
    ...
    public void paymentMethodsTest() {
        ...
        softAssert.assertEquals(paymentMethodsView.getPeetsCardHeader(), TestConstants.MobileTestData.PEETS_CARD_HEADER,"Peets Card header is not displayed");
        softAssert.assertEquals(paymentMethodsView.getSavedPaymentHeader(), TestConstants.MobileTestData.SAVED_PAYMENT_HEADER, "Saved Payment Methods header is not displayed");
        ...
        CreditCardDetailsView creditCardDetailsView = paymentMethodsView.clickSavedPaymentMethod();
        softAssert.assertEquals(creditCardDetailsView.getHeader(), TestConstants.MobileTestData.CC_NAME, "CC Name does not match title");
        softAssert.assertEquals(creditCardDetailsView.getCreditCardName(), TestConstants.MobileTestData.CC_NAME, "CC Name does not match");
        ...
        creditCardDetailsView = paymentMethodsView.clickSavedPaymentMethod();
        softAssert.assertEquals(creditCardDetailsView.getExpDate(), TestConstants.MobileTestData.CC_MODIFIED_EXP_DATE, "Exp date did not update");
