...
public class HomePageHoverItemsTest extends BaseTest {
	...
	@Test(groups = { Constants.TestNGGroups.HOME_PAGE }, description = "11107439")
	public void homepageEndofPageDescriptionTest() {
		...
		Assert.assertEquals(footer.getEndOfThePageDescription(), Constants.TestData.END_OF_PAGE_DESCRIPTION,
				"End of the page Description Mismatches");
	}

	@Test(groups = { TestNGGroups.WEB_REGRESSION }, description = "11107440")
	public void hoverCoffeOptions() {
		...
		for (CoffeeSubMenu menuItem : CoffeeSubMenu.values()) {
			...
			softAssert.assertTrue(WebHelper.getCurrentUrl().contains(menuItem.getLink()),
		...
		softAssert.assertTrue(WebHelper.getCurrentUrl().contains(TestData.COFFEE_HOVER_BLEND),
				"New Page is not redirected: " + WebHelper.getCurrentUrl());
		...
		softAssert.assertTrue(WebHelper.getCurrentUrl().contains(TestData.COFFEE_HOVER_SUMATRA),
				"New Page is not redirected: " + WebHelper.getCurrentUrl());
		...
		if (WebHelper.isDesktop()) {
			...
			softAssert.assertTrue(WebHelper.getCurrentUrl().contains(TestData.COFFEE_HOVER_FINDER),
					"New Page is not redirected: " + WebHelper.getCurrentUrl());
		}
		softAssert.assertAll();
	}

	@Test(groups = { TestNGGroups.WEB_REGRESSION }, description = "11107441")
	public void hoverTeaOptions() {
		...
		softAssert.assertTrue(WebHelper.getCurrentUrl().contains(TestData.TEA_HOVER_FINDER),
				"New Page is not redirected: " + WebHelper.getCurrentUrl());
		...
		softAssert.assertTrue(WebHelper.getCurrentUrl().contains(TestData.TEA_HOVER_MIGHTY_LEAF),
				"New Page is not redirected: " + WebHelper.getCurrentUrl());
