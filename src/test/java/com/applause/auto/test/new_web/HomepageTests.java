...
public class HomepageTests extends BaseTest {
	...
	@Test(groups = { TestNGGroups.WEB_REGRESSION }, description = "11107418")
	public void peetsLogoandHeader() {
		...
		softAssert.assertTrue(header.isVisitUsMenuItemDisplayed(), "Visit Us menu item is not displayed.");
		logger.info("-- Free Home Delivery");
		softAssert.assertTrue(header.isFreeHomeDeliveryMenuItemDisplayed(),
		...
		softAssert.assertTrue(header.isPeetnikRewardsMenuItemDisplayed(),
	...
	@Test(groups = { TestNGGroups.WEB_REGRESSION }, description = "11107419")
	public void headerOptions() {
		...
		for (CoffeeSubMenu menuItem : CoffeeSubMenu.values()) {
			...
			softAssert.assertTrue(header.isSubMenuItemDisplayed(menuItem.getLink()),
		...
		for (VisitUsSubMenu menuItem : VisitUsSubMenu.values()) {
			logger.info("Verifying: " + menuItem.getText());
			softAssert.assertTrue(header.isAlternativeSubMenuItemDisplayed(menuItem.getLink()),
					menuItem.getText() + " did not display.");
		}
		...
		softAssert.assertTrue(freeHomePage.isPageHeadingDisplayed(),
		...
		softAssert.assertTrue(peetnikRewardsPage.isPageHeadingDisplayed(),
		...
		softAssert.assertTrue(currentOffersPage.isPageHeadingDisplayed(),
				"Offers did not bring us to the correct page");

		softAssert.assertAll();
	}

	@Test(groups = { TestNGGroups.WEB_REGRESSION }, description = "11107420")
	public void peetsHomepageBannerandPromoTiles() {
		...
		softAssert.assertTrue(
				...
				"Carousel button brought us to a page that does not match the location.  Got: "
						+ plpPage.getDriver().getCurrentUrl() + ", expected: "
		...
		for (int tile = 1; tile <= 8; tile++) {
			softAssert.assertTrue(homePage.isPromoTileActionsVisible(tile),
					"Promo actions at position " + tile + " was not visible.");
			softAssert.assertTrue(homePage.isPromoTileTitleVisible(tile),
					"Promo title at position " + tile + " was not visible.");
			softAssert.assertTrue(homePage.isPromoTileDescriptionVisible(tile),
					"Promo description at position " + tile + " was not visible.");
			softAssert.assertEquals(PromoTiles.values()[tile - 1].getTitle().toLowerCase(),
					homePage.getPromoTileTitleText(tile).toLowerCase(),
					"Promo title at position " + tile + " did not match expectation.  Got "
							+ homePage.getPromoTileTitleText(tile).toLowerCase() + ", expected "
							+ PromoTiles.values()[tile - 1].getTitle().toLowerCase());
			softAssert.assertEquals(PromoTiles.values()[tile - 1].getDescription().toLowerCase(),
					homePage.getPromoTileDescriptionText(tile).toLowerCase(),
					"Promo description at position " + tile + " did not match expectation.  Got "
							+ homePage.getPromoTileDescriptionText(tile).toLowerCase() + ", expected "
							+ PromoTiles.values()[tile - 1].getDescription().toLowerCase());
			softAssert.assertEquals(PromoTiles.values()[tile - 1].getActions().toLowerCase(),
					homePage.getPromoTileActionsText(tile).toLowerCase(),
					"Promo title at position " + tile + " did not match expectation.  Got "
							+ homePage.getPromoTileActionsText(tile).toLowerCase() + ", expected "
	...
	@Test(groups = { TestNGGroups.WEB_REGRESSION }, description = "11107421")
	public void homepageShopCoffeeShopTea() {
		...
		softAssert.assertTrue(
				...
				"Coffee Promo tile brought us to wrong page.  Got " + plpPage.getDriver().getCurrentUrl()
						+ ", expected: " + Constants.HomepagePromoTileData.COFFEE_PROMO_LINK);
		logger.info(" -- navigating back to home");
		homePage = navigateToHome();
		...
		softAssert.assertTrue(
				...
				"Tea promo tile brought us to wrong page.  Got " + plpPage.getDriver().getCurrentUrl() + ", expected: "
						+ Constants.HomepagePromoTileData.TEA_PROMO_LINK);
		logger.info(" -- navigating back to home");
		homePage = navigateToHome();

		logger.info("4. Validate description for shop coffee section");
		softAssert.assertEquals(homePage.getCoffeePromoTileDescription(),
				Constants.HomepagePromoTileData.COFFEE_PROMO_DESCRIPTION,
				"Description for coffee did not match.  Got: " + homePage.getCoffeePromoTileDescription()
						+ ", expected: " + Constants.HomepagePromoTileData.COFFEE_PROMO_DESCRIPTION);
		logger.info("5. Validate description for shop tea section");
		softAssert.assertEquals(homePage.getTeaPromoTileDescription(),
				Constants.HomepagePromoTileData.TEA_PROMO_DESCRIPTION,
				"Description for tea did not match.  Got: " + homePage.getTeaPromoTileDescription() + ", expected: "
						+ Constants.HomepagePromoTileData.TEA_PROMO_DESCRIPTION);

		softAssert.assertAll();
	}

	@Test(groups = { TestNGGroups.WEB_REGRESSION }, description = "11107422")
	public void homepageBestSellers() {
		...
		softAssert.assertTrue(
				...
				"Best sellers button brought us to wrong location.  Got: " + plpPage.getDriver().getCurrentUrl()
						+ ", expected: " + Constants.HomepageBestSellersData.BEST_SELLERS_LINK);
		...
		for (int i = 0; i < homePage.getBestSellersPositionLength(); i++) {
			softAssert.assertNotEquals("", homePage.getBestSellersPositionTitle(firstPosition),
					"First Best seller was empty.");
			softAssert.assertNotEquals(shownPositions.get(i), homePage.getBestSellersPositionTitle(i),
		...
		for (int i = 0; i < homePage.getBestSellersPositionLength(); i++) {
			softAssert.assertEquals(shownPositions.get(i), homePage.getBestSellersPositionTitle(i),
		...
		softAssert.assertTrue(homePage.isBestSellersQuickAddDisplayedOnHover(),
