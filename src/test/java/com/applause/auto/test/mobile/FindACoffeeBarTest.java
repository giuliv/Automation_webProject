...
public class FindACoffeeBarTest extends BaseTest {
	...
	@Test(groups = { TestNGGroups.ORDER, TestNGGroups.DAILY, TestNGGroups.REGRESSION }, description = "1687260")
	@ApplauseTestCaseId({ "674542", "674541" })
	public void findACoffeeBarTest() {
		logger.info("PRECONDITION - User is signed in to app\n" + "User is on find a coffeebar screen\n"
		...
		AllowLocationServicesPopupChunk allowLocationServicesPopupChunk = dashboardView.getBottomNavigationMenu()
				.order(AllowLocationServicesPopupChunk.class).allowIfRequestDisplayed(OrderView.class)
				.locateCoffeebars(AllowLocationServicesPopupChunk.class);
		NearbySelectCoffeeBarView nearbySelectCoffeeBarView = allowLocationServicesPopupChunk.allowIfRequestDisplayed();
		...
		Assert.assertTrue(storeDetails.isNavigateBackDisplayed(),
		...
		Assert.assertTrue(storeDetails.isCoffeebarPinDisplayedOnMap(),
		...
		Assert.assertTrue(storeDetails.getCoffeebarSubHeaderName().length() > 0,
		...
		Assert.assertTrue(storeDetails.isCoffeebarFavorite(),
		...
		Assert.assertTrue(storeDetails.getCoffeebarLocation().length() > 0,
				"Address of the coffeebar and distance [x.x miles away] field does not displayed");

		logger.info("VERIFY - Sub header: HOURS");
		Assert.assertTrue(storeDetails.isCoffeebarSubHeaderHoursDisplayed(), "Sub header: HOURS does not displayed");

		logger.info("VERIFY - Open until x:xx PM");
		Assert.assertTrue(storeDetails.isCoffeebarOpenHoursDisplayed(), "Open until x:xx PM does not displayed");

		logger.info("VERIFY - Table with columns: Day and Store Hours (or Closed if applicable)");
		Assert.assertTrue(storeDetails.isCoffeebarDayAndStoreHoursDisplayed(),
		...
		Assert.assertTrue(storeDetails.isDirectionsButtonDisplayed(), "[Button] Directions does not displayed");
		...
		Assert.assertEquals(orderView.getHeadingTextValue().toLowerCase().toUpperCase(), "ORDER",
		...
		Assert.assertEquals(orderView.getStoreName(), storeName,
				"Coffeebar name is correct on location field of order screen");
	}

	@Test(groups = { TestNGGroups.ORDER, TestNGGroups.DAILY, TestNGGroups.REGRESSION }, description = "1687261")
	...
	public void recentCoffeeBarTest() {
		...
		logger.info("VERIFY - Coffeebars where user recently placed orders appear vertically below recents tab");
		...
		Assert.assertTrue(recent.isCoffeebarStoreNameDisplayed("AppInt Sandbox 1"), "Store name does not displayed");
		...
		Assert.assertTrue(recent.isCoffeebarLocationDisplayed("10900 Lakeline Mall Drive\nAustin, TX 78717"),
		...
		logger.info("VERIFY - Make sure coffeebar name is correct on location field of order screen");
		Assert.assertEquals(order.getStoreName().toUpperCase(), "AppInt Sandbox 1".toUpperCase(),
				"Wrong storename displayed");
	}

	@Test(groups = { TestNGGroups.ORDER, TestNGGroups.DAILY, TestNGGroups.REGRESSION }, description = "1687262")
	...
	public void favoriteCoffeeBarsTest() {
		...
		CoffeeStoreContainerChuck coffeeStore = nearbySelectCoffeeBarView.getCoffeeStoreContainerChuck();
		...
		Assert.assertTrue(storeDetailsView.isCoffeebarFavorite(), "Favorite was not enabled for coffee store");
		...
		logger.info("VERIFY - The coffeeBar that was favorited in step 3 should appear in the list of favorite stores");
		CoffeeStoreContainerChuck favStore = findACoffeeBarView.getCoffeeStoreContainerChuck();

		Assert.assertTrue(favStore.getStoreName().toUpperCase().contains(storeName.toUpperCase()),
				"Wrong store shown under favorites tab:" + storeName);
		...
		softAssert.assertTrue(storeDetailsView.isCoffeebarFavorite(), "Favorite was not enabled for coffee store");
		...
		softAssert.assertFalse(storeDetailsView.isCoffeebarFavorite(), "Favorite was not disabled for coffee store");
