...
public class CreateAccountTest extends BaseTest {
	...
	@Test(groups = { TestNGGroups.ONBOARDING, TestNGGroups.REGRESSION, TestNGGroups.WEB_UI }, description = "625879")
	...
	@Test(groups = { TestNGGroups.ONBOARDING, TestNGGroups.REGRESSION,
			TestNGGroups.MONITORING }, description = "625883")
	public void signInEmailPasswordTest() {
		...
		Assert.assertTrue(!signInView.getPassword().equals(TestData.PASSWORD), "Password does not hidden");

		logger.info("Tap on show password icon");
		signInView.showPassword();

		logger.info("Make sure password entered is displayed to user");
		Assert.assertEquals(signInView.getUnEncryptedPassword(), MyAccountTestData.PASSWORD, "Password does not shown");
		...
		HomeView homeView = signInView.signIn(HomeView.class).getReorderTooltipComponent()
				.closeReorderTooltipIfDisplayed(HomeView.class).getCheckInTooltipComponent()
				.closeCheckInTooltipIfDisplayed(HomeView.class).getPointsTurnIntoRewardsTooltipComponent()
	...
	@Test(groups = { TestNGGroups.ONBOARDING, TestNGGroups.REGRESSION }, description = "625925")
	public void accountSettingsEditProfileTest() {
		SoftAssert softAssert = new SoftAssert();
		HomeView homeView = TestHelper.skipOnboardingAndLogin(MyAccountTestData.EDIT_EMAIL,
		...
		softAssert.assertTrue(profileDetailsView.isHeaderDisplayed(), "Header isn't displayed properly");

		logger.info("Verify - Back arrow at top left corner is displayed");
		softAssert.assertTrue(profileDetailsView.isBackButtonDisplayed(), "Back button isn't displayed");

		logger.info("Verify - First name field is displayed");
		softAssert.assertTrue(profileDetailsView.isFirstNameFieldDisplayed(), "First name field isn't displayed");

		logger.info("Verify - Last name field is displayed");
		softAssert.assertTrue(profileDetailsView.isLastNameFieldDisplayed(), "Last name field isn't displayed");

		logger.info("Verify - Date of birth field (not editable) is displayed");
		softAssert.assertTrue(profileDetailsView.isBirthdayFieldDisplayed(), "Birthday field isn't displayed");

		logger.info("Verify - Phone number field is displayed");
		softAssert.assertTrue(profileDetailsView.isPhoneNumberFieldDisplayed(), "Phone number field isn't displayed");

		logger.info("Verify - Email address field is displayed");
		softAssert.assertTrue(profileDetailsView.isEmailFieldDisplayed(), "Email field isn't displayed");

		logger.info("Verify - Change password link is displayed");
		softAssert.assertTrue(profileDetailsView.isChangePasswordLinkAvailable(),
				"Change Password link isn't displayed");
		...
		String firstNameNew = firstNameOrig.matches("ApplauseUpdated$") ? "Applause" : "ApplauseUpdated";
	...
	@Test(groups = { TestNGGroups.ONBOARDING, TestNGGroups.REGRESSION }, description = "625927")
	public void accountSettingsGeneralSettingsTest() {
		SoftAssert softAssert = new SoftAssert();
		HomeView homeView = TestHelper.skipOnboardingAndLogin(MyAccountTestData.EMAIL, MyAccountTestData.PASSWORD);
		Assert.assertNotNull(homeView, "Home View does not displayed");
		...
		Assert.assertTrue(personalSettingsView.isPromoEmailOptionChecked(), "Promo emails does not turned on");
		...
		Assert.assertFalse(personalSettingsView.isPromoEmailOptionChecked(), "Promo emails does not turned off");
		...
		softAssert.assertTrue(personalSettingsView.isLocationServicesChecked(), "Location isn't enabled");
		...
		softAssert.assertFalse(personalSettingsView.isLocationServicesChecked(), "Location isn't disabled");

		softAssert.assertAll();
	}

	@Test(groups = { TestNGGroups.ONBOARDING, TestNGGroups.REGRESSION }, description = "625926")
	public void accountSettingsChangePasswordTest() {
		...
		homeView.getReorderTooltipComponent().closeReorderTooltipIfDisplayed(HomeView.class)
				...
				.getPointsTurnIntoRewardsTooltipComponent().closeTooltipIfDisplayed(HomeView.class);
		Assert.assertNotNull(homeView, "Home View does not displayed");
		...
		softAssert.assertTrue(changePasswordView.isChangePasswordHeaderDisplayed(),
				"CHANGE PASSWORD header is not displayed");

		logger.info("VERIFY - Back arrow button is displayed");
		softAssert.assertTrue(changePasswordView.isBackArrowButtonDisplayed(), "Back arrow button is not displayed");

		logger.info("VERIFY - Current password field with show/hide password icon is displayed");
		softAssert.assertTrue(changePasswordView.isCurrentPasswordFieldDisplayed(), "Current filed isn't displayed");

		logger.info("VERIFY - New password field with show/hide password icon is displayed");
		softAssert.assertTrue(changePasswordView.isNewPasswordFieldDisplayed(), "New filed isn't displayed");

		logger.info("VERIFY - Change Password button is displayed");
		softAssert.assertTrue(changePasswordView.isChangePasswordButtonDisplayed(),
		...
		logger.info("VERIFY - User sees success check mark and a UI alert: Your new password has been set [Okay]");
		...
		logger.info("STEP - Tap Sign In button");
		signInView.showPassword();
		homeView = signInView.signIn(HomeView.class).getPointsTurnIntoRewardsTooltipComponent()
				.closeTooltipIfDisplayed(HomeView.class);
	...
	@Test(groups = { TestNGGroups.ONBOARDING, TestNGGroups.REGRESSION,
			TestNGGroups.MONITORING }, description = "625880")
	public void createAccountEmailPassword() {
		...
		Assert.assertFalse(createAccountView.isEmailOptInChecked(), "Email opt in checkbox remains marked");
		createAccountView.tapEmailOptIn();
		Assert.assertTrue(createAccountView.isEmailOptInChecked(), "Email opt in checkbox does not marked");
		...
		logger.info("User sees a loading dial, then a check mark to indicate successful account creation");
		HomeView homeView = createAccountView.tapOnCreateAccount().getSwipeTooltipComponent()
				.closeTooltipIfDisplayed(HomeView.class).getPointsTurnIntoRewardsTooltipComponent()
				.closeTooltipIfDisplayed(HomeView.class);
		 ...
		 *
		 * <p>
		 * Need to check this step. Currently there is no offers
		 *
		 * <p>
		 * logger.info("Swipe through dashboard feed"); logger.info("User sees offer $2 OFF OA TEST in
		 * dashboard"); Assert.assertTrue( dashboardView.lookUpOffer("$2 OFF OA TEST"), "User does not see
		 * 'offer $2 OFF OA TEST'");
		 */
		...
		Assert.assertEquals(dateOfBirthday, String.format("%s %s, %s", dobMonth, dobDay, dobYear),
				"Date does not match");
		Assert.assertEquals(emailUpd, email, "Email does not match");
		Assert.assertTrue(profileDetailsView.isChangePasswordLinkAvailable(),
		...
		logger.info("Make sure promotional emails toggle reflects whatever selection user chose at step 14");
		Assert.assertTrue(personalSettingsView.isPromoEmailOptionChecked(), "Promo email does not checked");
	...
	@Test(groups = { TestNGGroups.ONBOARDING, TestNGGroups.REGRESSION }, description = "2980586")
	public void createAccountFieldValidation() {
		logger.info("Launch the app and arrive at the first onboarding screen view");
		LandingView landingView = testHelper.navigateToLandingView();

		Assert.assertEquals(landingView.getHeadingTextValue(), "Peetnik Rewards",
		...
		Assert.assertTrue(createAccountView.isFirstnameDisplayed(), "Firstname field does not displayed");
		Assert.assertTrue(createAccountView.isLastDisplayed(), "Lastname field does not displayed");
		Assert.assertTrue(createAccountView.isDobTextDisplayed(), "Birthday drink text does not displayed");
		Assert.assertTrue(createAccountView.isEmailAddressDisplayed(), "Email address field does not displayed");
		...
		Assert.assertTrue(createAccountView.isPromocodeTextDisplayed(),
				"Promo code triggered text field does not displayed");
		Assert.assertTrue(createAccountView.isEmailOptInChecked(), "Email opt-in does not checked by default");
		Assert.assertFalse(createAccountView.isPrivacyPolicyAndTermsAndConditionsChecked(),
	...
	@Test(groups = { TestNGGroups.ONBOARDING, TestNGGroups.REGRESSION }, description = "625882")
	public void createAccountExistingWebUserTest() {
		...
		SignUpUserDto newUser = SignUpUserDto.builder().email(TestData.USERNAME_625882).password(TestData.PASSWORD)
				.build();
		ConfirmationPopup confirmationPopup = testHelper.createNewAccount(createAccountView, newUser,
				ConfirmationPopup.class);

		logger.info("Verify that User should be served error message");
		Assert.assertEquals(confirmationPopup.getPopupMessage(), MobileTestData.ACCOUNT_ALREADY_EXIST_MESSAGE,
		...
		logger.info("Tap Sign In button");
		HomeView homeView = signInView.signIn(HomeView.class).getPointsTurnIntoRewardsTooltipComponent()
				.closeTooltipIfDisplayed(HomeView.class).getPointsTurnIntoRewardsTooltipComponent()
				.closeTooltipIfDisplayed(HomeView.class).getSwipeTooltipComponent()
				.closeTooltipIfDisplayed(HomeView.class);
	...
	@Test(groups = { TestNGGroups.ONBOARDING, TestNGGroups.REGRESSION }, description = "625929")
	public void accountSettingsAccountHistoryTest() {
		...
		softAssert.assertTrue(accountActivityView.isBackButtonDisplayed(), "Back button is not displayed");

		logger.info("VERIFY - Back 'ACCOUNT ACTIVITY' header is displayed");
		softAssert.assertTrue(accountActivityView.isHeaderDisplayed(), "'ACCOUNT ACTIVITY' header is not displayed");
