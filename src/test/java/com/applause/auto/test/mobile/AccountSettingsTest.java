...
public class AccountSettingsTest extends BaseTest {
	...
	@Test(groups = {TestNGGroups.ACCOUNT_SETTINGS }, description = "625928")
	public void paymentMethodsTest() {
		...
		softAssert.assertEquals(paymentMethodsView.getSavedPaymentHeader(),
				MobileTestData.SAVED_PAYMENT_HEADER, "Saved Payment Methods header is not displayed");
		...
		softAssert.assertEquals(creditCardDetailsView.getHeader(), MobileTestData.CC_NAME,
				"CC Name does not match title");
		softAssert.assertEquals(creditCardDetailsView.getCreditCardName(), MobileTestData.CC_NAME,
		...
		softAssert.assertEquals(creditCardDetailsView.getExpDate(), MobileTestData.CC_MODIFIED_EXP_DATE,
