...
public class MobileHelper {
	...
	public static void initMobileBrowser() {
		if (SdkHelper.getEnvironmentHelper().isMobileAndroid()) {
			...
			boolean isChromeBrowserStarted = currentActivity.equals("com.android.chrome")
					|| currentActivity.contains("ChromeTabbedActivity");
			boolean isIntentActionStarted = currentActivity.equals("com.android.internal.app.ResolverActivity");

			logger.info("Current activity: " + currentActivity);

			if (isIntentActionStarted) {
				((AppiumDriver) SdkHelper.getDriver()).findElementByXPath("//android.widget.TextView[@text='Chrome']")
						.click();
	 ...
	 *            offset from element center to the right in percents of element width. Value should be
	 *            from 0 to 1
	 * @param yRelativeOffset
	 *            offset from element center to the top in percents of element height. Value should be
	 *            from 0 to 1
	 */
	public static void tapOnElementWithOffset(BaseElement element, double xRelativeOffset, double yRelativeOffset) {
		int xCenter = element.getMobileElement().getCenter().x;
		int yCenter = element.getMobileElement().getCenter().y;
		logger.info(String.format("Add this offset to click: x = [%f] , y = [%f]", xRelativeOffset, yRelativeOffset));
		...
		logger.info(String.format("Clicking with offset: x = [%d] , y = [%d]", xAbsoluteOffset, yAbsoluteOffset));
	...
	public static void swipeAcrossScreenCoordinates(double startX, double startY, double endX, double endY,
			long millis) {
		...
		logger.info("Scrolling from: [" + startX_ + " , " + startY_ + "] to [" + endX_ + " , " + endY_ + "]");
	...
	public static void scrollUntilElementSectionWillBeAvailableOnTheScreenInWebView(BaseElement element,
			String elementName, int maxSwipingAttempts) {
		...
		while (currentSwipingAttempts <= maxSwipingAttempts) {
			try {
				...
				SdkHelper.getDeviceControl().swipeAcrossScreenCoordinates(screenWidth / 2, (int) (screenHeight * 0.75),
						screenWidth / 2, (int) (screenHeight * 0.4), 1000);
				SdkHelper.getSyncHelper().sleep(5000);
				currentSwipingAttempts++;
				logger.info("XML Dump: ", SdkHelper.getDriver().getPageSource());
			}
	...
	public static BufferedImage getMobileScreenshotBufferedImage() {
		...
		if (ArrayUtils.isNotEmpty(screenShot)) {
			...
			throw new IllegalStateException("Was not able to get mobile screenshot");
	}
	...
	public static RGB getMobileElementColour(MobileElement element) {
		...
		double centerY0 = (double) point.getY()
				/ (SdkHelper.getEnvironmentHelper().isMobileIOS() ? (double) dimension.height
						: ((double) dimension.height + screenShotHeight - screenHeight)); // correction due to top nav
		// bar

		// Getting pixel color by position x and y
		int clr = image.getRGB((int) (image.getWidth() * centerX0), ((int) (image.getHeight() * centerY0)));
	...
	public static void scrollElementIntoView(BaseElement element) {
		...
		int screenHeight = SdkHelper.getDeviceControl().getScreenSize().height;
		IntStream.range(1, 6).filter(i -> {
			try {
				SdkHelper.getSyncHelper().sleep(2000);
				element.initialize();
				Dimension dimension = element.getDimension();
				Point location = element.getLocation();
				logger.info(String.format("Location [%s] height [%s]", location.y, dimension.height));
