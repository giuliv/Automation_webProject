...
public class CoffeeStoreItemChuck extends BaseComponent {
	...
	/* -------- Elements -------- */
	@Locate(xpath = "./XCUIElementTypeOther/XCUIElementTypeButton/XCUIElementTypeButton[1]", on = Platform.MOBILE_IOS)
	@Locate(xpath = "//*[contains(@resource-id, 'com.wearehathway.peets.development:id/storeDetail') and descendant::*[contains(@resource-id,'storeDetailContainer')]]"
			+ "//*[contains(@resource-id, 'storeName')]", on = Platform.MOBILE_ANDROID)
	protected Text getStoreDetailsItem;

	@Locate(xpath = "//XCUIElementTypeButton[(@name=\"Order\" or @name=\"Reorder\") and @visible=\"true\"]", on = Platform.MOBILE_IOS)
