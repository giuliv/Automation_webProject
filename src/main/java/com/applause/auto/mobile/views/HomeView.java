...
public class HomeView extends BaseComponent {
	...
	@Getter
	@Locate
	ReorderTooltipComponent reorderTooltipComponent;
	@Getter
	@Locate
	CheckInTooltipComponent checkInTooltipComponent;
	@Getter
	@Locate
	PointsTurnIntoRewardsTooltipComponent pointsTurnIntoRewardsTooltipComponent;
	@Getter
	@Locate
	SwipeTooltipComponent swipeTooltipComponent;
	@Getter
	@Locate
	BottomNavigationMenuChunk bottomNavigationMenuChunk;

	@Locate(iOSClassChain = "**/XCUIElementTypeStaticText[`label == 'Home'`]", on = Platform.MOBILE_IOS)
	@Locate(androidUIAutomator = "new UiSelector().resourceIdMatches(\".*id/homeButton\")", on = Platform.MOBILE_ANDROID)
	protected Button homeButton;

	@Locate(iOSClassChain = "**/XCUIElementTypeStaticText[`label == 'Menu'`]", on = Platform.MOBILE_IOS)
	@Locate(androidUIAutomator = "new UiSelector().resourceIdMatches(\".*id/menuButton\")", on = Platform.MOBILE_ANDROID)
	protected Button menuButton;

	@Locate(iOSClassChain = "**/XCUIElementTypeStaticText[`label == 'More'`]", on = Platform.MOBILE_IOS)
	@Locate(androidUIAutomator = "new UiSelector().resourceIdMatches(\".*id/moreButton\")", on = Platform.MOBILE_ANDROID)
	protected Button moreButton;

	@Locate(xpath = "//XCUIElementTypeStaticText[@value = 'Swipe left or tap the icon to scan your QR code in-store.']|//XCUIElementTypeButton[@label = 'QR Code']", on = Platform.MOBILE_IOS)
	@Locate(androidUIAutomator = "new UiSelector().resourceIdMatches(\".*id/dashboard_motion_layout\")", on = Platform.MOBILE_ANDROID)
	...
	public <T extends BaseComponent> T tapOnStoresButton(Class<T> clazz) {
		...
		SdkHelper.getSyncHelper().wait(Until.uiElement(storesButton).clickable().setTimeout(Duration.ofSeconds(50)));
		storesButton.click();
