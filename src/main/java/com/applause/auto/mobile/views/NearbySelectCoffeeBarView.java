...
public class NearbySelectCoffeeBarView extends BaseComponent {
	...
	@Locate(xpath = "//XCUIElementTypeButton[(@name='ORDER' or @name='Order') and @visible='true']", on = Platform.MOBILE_IOS)
	...
	@Locate(xpath = "//XCUIElementTypeNavigationBar[@name=\"FIND A COFFEEBAR\"]", on = Platform.MOBILE_IOS)
	@Locate(id = "com.wearehathway.peets.development:id/storeContainer", on = Platform.MOBILE_ANDROID)
	protected ContainerElement getSignatureForFindCoffee;

	@Locate(xpath = "//XCUIElementTypeStaticText[@name=\"Enter Zip or City, State\"]/following-sibling::XCUIElementTypeImage", on = Platform.MOBILE_IOS)
	@Locate(id = "com.wearehathway.peets.development:id/storeSearchIcon", on = Platform.MOBILE_ANDROID)
	...
	@Locate(androidUIAutomator = "new UiSelector().resourceIdMatches(\".*searchRow\"). childSelector(new UiSelector().classNameMatches(\".*TextView\"))", on = Platform.MOBILE_ANDROID)
	protected TextBox getSearchTextBox;

	@Locate(id = "Search for coffeebar edit field", on = Platform.MOBILE_IOS)
	@Locate(androidUIAutomator = "new UiSelector().resourceIdMatches(\".*searchField\")", on = Platform.MOBILE_ANDROID)
	...
	@Locate(iOSClassChain = "**/XCUIElementTypeButton[`label == \"Nearby\"`]", on = Platform.MOBILE_IOS)
	protected TextBox nearbyTab;

	@Locate(xpath = "//android.widget.TextView[contains(@text,\"Favorites\")]", on = Platform.MOBILE_ANDROID)
	...
	@Locate(iOSClassChain = "**/XCUIElementTypeNavigationBar/XCUIElementTypeStaticText", on = Platform.MOBILE_IOS)
	...
	@Locate(xpath = "//XCUIElementTypeMap/following-sibling::XCUIElementTypeOther/XCUIElementTypeOther[@value='Store']", on = Platform.MOBILE_IOS)
	@Locate(xpath = "//android.view.View[@content-desc='Google Map. Use standard google maps touch gestures to move the map. Location details are located after the map.']/android.view.View[ends-with(@content-desc,'Location. ')]", on = Platform.MOBILE_ANDROID)
	protected LazyList<Button> pinsOnMapButton;

	@Locate(xpath = "//XCUIElementTypeButton[@name=\"user location\"]/following-sibling::XCUIElementTypeCollectionView/XCUIElementTypeCell", on = Platform.MOBILE_IOS)
	...
	@Locate(id = "com.wearehathway.peets.development:id/directionButton", on = Platform.MOBILE_ANDROID)
	...
	public void afterInit() {
		try {
			...
			getSyncHelper()
					.wait(Until.uiElement(dismissFreeDeliveryButton).present().setTimeout(Duration.ofSeconds(30)));
		...
		try {
			...
			getSyncHelper()
					.wait(Until.uiElement(getSignatureForFindCoffee).present().setTimeout(Duration.ofSeconds(45)));
...
class AndroidNearbySelectCoffeeBarView extends NearbySelectCoffeeBarView {
	...
	public NearbySelectCoffeeBarView search(String searchTxt) {
		logger.info("Searching for store: " + searchTxt);
		getSearchTextBox.click();
		getSyncHelper().wait(Until.uiElement(getSearchTextBoxEdit).present().setTimeout(Duration.ofSeconds(3)));
