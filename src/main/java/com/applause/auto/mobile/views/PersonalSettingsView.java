...
public class PersonalSettingsView extends BaseComponent {
	...
	@Locate(iOSClassChain = "**/XCUIElementTypeNavigationBar[`name == 'PERSONAL SETTINGS'`]/XCUIElementTypeButton", on = Platform.MOBILE_IOS)
	...
	@Locate(xpath = "//XCUIElementTypeApplication[@name=\"Peets-Sandbox\"]/XCUIElementTypeWindow[1]/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther", on = Platform.MOBILE_IOS)
	...
	@Locate(iOSClassChain = "**/XCUIElementTypeButton[`label == \"Go to Settings\"`]", on = Platform.MOBILE_IOS)
	@Locate(xpath = "//android.widget.Button[@text='SETTINGS']", on = Platform.MOBILE_ANDROID)
	protected Button openDeviceSettingsButton;

	@Locate(xpath = "//XCUIElementTypeButton[contains(@name,'Allow') or contains(@name,'allow')]", on = Platform.MOBILE_IOS)
	@Locate(xpath = "//android.widget.Button[starts-with(@text,'Allow') or starts-with(@text,'allow') or starts-with(@text,'TOLAK') or @text='ALLOW']", on = Platform.MOBILE_ANDROID)
	protected Button allowPeetsAccessDeviceLocationButton;

	@Locate(iOSClassChain = "**/XCUIElementTypeCell[`label BEGINSWITH \"Location\"`]", on = Platform.MOBILE_IOS)
	...
	@Locate(iOSClassChain = "**/XCUIElementTypeCell[`label == \"While Using\"`]", on = Platform.MOBILE_IOS)
	...
	protected void openDeviceSettingsLocationOn() {
		...
		SdkHelper.getSyncHelper().sleep(5000);
		MobileHelper.openSettings().init().openMenuItem("Privacy", IosSettingsView.class)
...
class AndroidPersonalSettingsView extends PersonalSettingsView {
	...
	@Locate(xpath = "//*[@text=\"Allow only while using the app\" or @text=\"ALLOW ONLY WHILE USING THE APP\"]", on = Platform.MOBILE_ANDROID)
