...
public class IosSettingsView extends BaseComponent {
	...
	@Locate(iOSClassChain = "**/XCUIElementTypeCell[`label BEGINSWITH \"%s\"`]", on = Platform.MOBILE_IOS)
	...
	public IosSettingsView init() {
		...
		SdkHelper.getSyncHelper().wait(Until.uiElement(signature).visible().setTimeout(Duration.ofSeconds(30)));
		return this;
	}
