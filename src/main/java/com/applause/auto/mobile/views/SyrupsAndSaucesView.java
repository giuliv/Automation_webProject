...
@Implementation(is = IosSyrupsAndSaucesView.class, on = Platform.MOBILE_IOS)
public class SyrupsAndSaucesView extends BaseComponent {
	...
	@Locate(xpath = "//XCUIElementTypeStaticText[@name=\"%s\"]/following-sibling::XCUIElementTypeButton|//XCUIElementTypeCell/XCUIElementTypeStaticText[@name=\"%s\"]", on = Platform.MOBILE_IOS)
	protected Button syrupItemButton;

	@Locate(xpath = "//XCUIElementTypeCell/XCUIElementTypeStaticText[@name=\"%s\" and @visible='true']", on = Platform.MOBILE_IOS)
	@Locate(xpath = "//android.widget.TextView[@text='%s']", on = Platform.MOBILE_ANDROID)
...
class IosSyrupsAndSaucesView extends SyrupsAndSaucesView {
	@Override
	public SyrupsAndSaucesView selectSyrup(String syrup) {
		syrupItemButton.format(syrup, syrup);
		MobileHelper.scrollElementIntoView(syrupItemButton);
		syrupItemButton.click();
		return this;
	}
}
