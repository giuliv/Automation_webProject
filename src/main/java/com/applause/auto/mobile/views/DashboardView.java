...
public class DashboardView extends BaseComponent {
	...
	@Locate(iOSClassChain = "**/XCUIElementTypeStaticText[`name == \"%s\"`]", on = Platform.MOBILE_IOS)
	...
	public void afterInit() {
		try {
			SdkHelper.getSyncHelper()
					.wait(Until.uiElement(dismissFreeDeliveryButton).present().setTimeout(Duration.ofSeconds(10)));
		...
		SdkHelper.getSyncHelper().wait(Until.uiElement(getSignature).present().setTimeout(Duration.ofSeconds(45)));
	}
	...
	public <T extends BaseComponent> T location(Class<T> clazz) {
		...
		SdkHelper.getSyncHelper().wait(Until.uiElement(locationButton).clickable().setTimeout(Duration.ofSeconds(50)));
...
class IosDashboardView extends DashboardView {
	...
	public <T extends BaseComponent> T location(Class<T> clazz) {
		logger.info("Click on Location button");
		SdkHelper.getSyncHelper().wait(Until.uiElement(locationButton).present().setTimeout(Duration.ofSeconds(50)));
