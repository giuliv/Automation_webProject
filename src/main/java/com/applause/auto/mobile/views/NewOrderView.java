...
public class NewOrderView extends BaseComponent {
	...
	@Locate(xpath = "//XCUIElementTypeButton[@name=\"%s\"]/preceding-sibling::XCUIElementTypeOther/XCUIElementTypeButton[@name='%s']", on = Platform.MOBILE_IOS)
	...
	@Locate(xpath = "//XCUIElementTypeOther[child:: XCUIElementTypeImage and XCUIElementTypeStaticText[@name>0]]", on = Platform.MOBILE_IOS)
	...
	@Locate(id = "com.wearehathway.peets.development:id/changeLocationButton", on = Platform.MOBILE_ANDROID)
	protected Button getChangeStoreButton;

	@Locate(xpath = "//android.widget.TextView[@text='Recents']", on = Platform.MOBILE_ANDROID)
	@Locate(iOSClassChain = "**/XCUIElementTypeButton[`label == \"Recents\"`]", on = Platform.MOBILE_IOS)
	protected Button recentsTabButton;

	@Locate(xpath = "//android.widget.TextView[@text='No Recent Orders']", on = Platform.MOBILE_ANDROID)
	@Locate(iOSClassChain = "**/XCUIElementTypeStaticText[`label == \"No Recent Orders\"`]", on = Platform.MOBILE_IOS)
	protected Button titleNoRecentOrdersText;

	@Locate(xpath = "//android.widget.TextView[@text='No Favorited Orders']", on = Platform.MOBILE_ANDROID)
	@Locate(iOSClassChain = "**/XCUIElementTypeStaticText[`label == \"No Favorited Orders\"`]", on = Platform.MOBILE_IOS)
	protected Button titleNoFavoriteOrders;

	@Locate(xpath = "//android.widget.TextView[@text='Favorites']", on = Platform.MOBILE_ANDROID)
	@Locate(iOSClassChain = "**/XCUIElementTypeButton[`label == \"Favorites\"`]", on = Platform.MOBILE_IOS)
	protected Button favoritesTabButton;

	@Locate(xpath = "//android.widget.Button[@text=\"Start New Order\"]", on = Platform.MOBILE_ANDROID)
	@Locate(iOSClassChain = "**/XCUIElementTypeButton[`label == \"Start New Order\"`][1]", on = Platform.MOBILE_IOS)
	protected Button startNewOrderRecentsButton;

	@Locate(xpath = "//android.widget.Button[@text=\"Start New Order\"]", on = Platform.MOBILE_ANDROID)
	@Locate(iOSClassChain = "**/XCUIElementTypeButton[`label == \"Start New Order\"`][2]", on = Platform.MOBILE_IOS)
	...
	@Locate(iOSClassChain = "**/XCUIElementTypeStaticText[`label == \"Menu\"`][2]", on = Platform.MOBILE_IOS)
	...
	@Locate(iOSClassChain = "**/XCUIElementTypeButton[`label == \"Add to Order\"`]", on = Platform.MOBILE_IOS)
	...
	@Locate(xpath = "//XCUIElementTypeNavigationBar/following-sibling::XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther[3]/XCUIElementTypeStaticText", on = Platform.MOBILE_IOS)
	protected Text fabAmountText;

	@Locate(xpath = "//XCUIElementTypeStaticText[@name=\"Seasonal Favorites\"]/preceding-sibling::XCUIElementTypeOther/XCUIElementTypeCollectionView/XCUIElementTypeCell[@name=\"%s\"]", on = Platform.MOBILE_IOS)
	@Locate(xpath = "//androidx.appcompat.widget.LinearLayoutCompat[@resource-id='com.wearehathway.peets.development:id/seasonalProductContainer']/android.widget.TextView[@resource-id='com.wearehathway.peets.development:id/productNameTextView' and @text='%s']", on = Platform.MOBILE_ANDROID)
	protected Text seasonalFavoriteProductItemText;

	/* -------- Actions -------- */
	public void afterInit() {
		getSyncHelper().wait(Until.uiElement(menuSignatureText).present().setTimeout(Duration.ofSeconds(15)));
	}
	...
	public CheckoutView checkout() {
		...
		try {
			...
			getSyncHelper().wait(Until.uiElement(getConfirmStoreButton).present().setTimeout(Duration.ofSeconds(5)));
